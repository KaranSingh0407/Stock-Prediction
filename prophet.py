...
def get_historical_stock_price(stock):
    ...
    startDate = datetime.datetime(2013, 1, 4)
    endDate = datetime.datetime.now().date()
    #endDate = pd.to_datetime(date)
    #endDate = datetime.datetime(date)
...
def main():
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template("index.html", error="Index Not Found")
        ...
        forecast_today = forecasted_data[-(num_days+1)]
        ...
        return render_template("plot.html", forecast_to = round(forecast_today,1) ,original = round(original_end,2), forecast = round(forecast_start,2), stock_tinker = stock.upper())
